...
if __name__ == "__main__":
    ...
    for i in range(params_number):
        model.load_state_dict(torch.load(os.path.join(trajectory_dir, str(i) + ".pt", ), map_location=device))
    ...
    # from scratch
    model.load_state_dict(torch.load(os.path.join(trajectory_dir, str(0) + ".pt"), map_location=device))
