...
for i in range(0, numDocs):
    ...
    # to convert the hash value into an integer.

    for i in range(len(words) - 2):
        shingle = words[i] + " " + words[i + 1] + " " + words[i + 2]
        shingle = shingle.encode("utf-8")
        shingle = int.from_bytes(sha1(shingle).digest(), byteorder="big")
...
for docID in docNames:
    ...
    # You should use the hash functions defined above.

    for i in range(numHashes):
        minHash = nextPrime + 1
        for shingleID in shingleIDSet:
            hashValue = (a_list[i] * shingleID + b_list[i]) % nextPrime
...
for i in range(0, numDocs):
    ...
    for j in range(i + 1, numDocs):
        ...
        # Jsim = |A and B| / |A or B| = |A and B| / (|A| + |B| - |A and B|)
        # where |A| is the number of elements in set A.
        # You should use the signature1 and signature2 to calculate the similarity.
        # You should store the similarity value into estJSim.
        # You should use the getTriangleIndex function to get the index of the similarity value in the estJSim list.
        same_num = 0
        for k in range(numHashes):
            if signature1[k] == signature2[k]:
                same_num += 1
        JS = same_num / (2*numHashes - same_num)
        estJSim[getTriangleIndex(i, j)] = JS
        ##### end of TODO #####
...
for i in range(0, numDocs):
    for j in range(i + 1, numDocs):
        ...
        if estJ > threshold:
            ...
            # You should set the actual Jaccard similarity here.
            # You should use the formula:
            # Jsim = |A and B| / |A or B| = |A and B| / (|A| + |B| - |A and B|)
            # where |A| is the number of elements in set A.
            shingles1 = docsAsShingleSets[docNames[i]]
            shingles2 = docsAsShingleSets[docNames[j]]
            same_num = len(shingles1.intersection(shingles2))
            J = same_num / (len(shingles1) + len(shingles2) - same_num)
            ### end of TODO ###
