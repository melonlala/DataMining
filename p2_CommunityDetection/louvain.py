...
class Louvain:
    ...
    def delta_modularity(self, node: int, community: Community) -> float:
        ...
        ##################
        m = self.G.M
        k_i_in = community.node2comm_in_degree(node) + community.node2comm_out_degree(node)
        k_in = self.G.get_in_degree(node)
        k_out = self.G.get_out_degree(node)
        com_in = community.get_in_degree()
        com_out = community.get_out_degree()
        delta_q = (k_i_in / m) - (k_in * com_out + k_out * com_in) / (m * m)
        # End of TODO
    ...
    def phase2(self):
        ...
        for metanode in self.G.nodes:
            for neighbor in self.G.get_out_neighbors(metanode):
                src = self.metanode2commid[metanode]
                dst = self.metanode2commid[neighbor]
                new_edges[src, dst] += self.G.get_edge_weight(metanode, neighbor)
        # End of TODO
    ...
    def merge_communities(
        ...
        # Your Code Here #
        ##################
        num_communities = len(set(node2commid.values()))
        label_count_per_community = [
            [0] * n_expected_communities for _ in range(num_communities)
        ]
        
        # Populate the matrix with label counts
        for node in gt_map:
            label = gt_map[node]
            comm_id = node2commid[node]
            if label is not None:
                label_count_per_community[comm_id][label] += 1

        comm_label = [label_count_per_community[i].index(max(label_count_per_community[i])) for i in range(num_communities)]
        for node, comm_id in node2commid.items():
            node2commid[node] = comm_label[comm_id]
        # End of TODO
        return node2commid
