...
class Community:
    ...
    def node2comm_in_degree(self, node: int) -> int:
        ...
        ##################
        d_in = 0
        in_neighbors = self.graph.get_in_neighbors(node)
        for in_neighbor in in_neighbors:
            if in_neighbor in self.nodes:
                d_in += self.graph.get_edge_weight(in_neighbor, node)
        return d_in
    ...
    def node2comm_out_degree(self, node: int) -> int:
        ...
        ##################
        d_out = 0
        out_neighbors = self.graph.get_out_neighbors(node)
        for out_neighbor in out_neighbors:
            if out_neighbor in self.nodes:
                d_out += self.graph.get_edge_weight(node, out_neighbor)
