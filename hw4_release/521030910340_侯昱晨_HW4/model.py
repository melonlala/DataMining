...
class CodeNN(nn.Module):
    ...
    def code_features(
        ...
        code_features = torch.cat(
            [
                self.method_name_encoder(method_ids, method_padding_mask),
                self.api_seq_encoder(apiseq_ids, apiseq_padding_mask),
                self.token_encoder(token_ids, token_padding_mask),
            ],
            dim=-1,
        )
        code_features = self.fusion_mlp(code_features)
        # End of TODO
